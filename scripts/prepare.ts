...
async function main() {
  ...
  if (existsSync(resolve(root, "readme.md"))) {
    writeText(
      "readme.md",
      readText("readme.md").replace(/\bxxx\b/g, name as string),
    );
  }
  ...
  if (createRepo) {
    ...
    execSync("gh repo create --no-push --remote origin", {
      cwd: root,
      stdio: "inherit",
    });
  }
  ...
  if (runner === "bun") {
    ...
    pkg.devDependencies = Object.fromEntries(
      Object.entries(pkg.devDependencies as Record<string, string>).sort(
